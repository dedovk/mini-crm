...
- автоматичні денні та місячні підсумки в Google Sheets, зокрема окремий облік
  комісії Prom за оплату частинами; точна комісія запитується з детального API,
  а відсутня сума позначається як невирішена без приблизного тарифного розрахунку;
...
`dry-run`, а параметр `installment_backfill_days` запускає історичну звірку комісій
Prom за вказану кількість днів. Concurrency-група не дозволяє двом запускам одночасно
записувати дані в таблицю.
